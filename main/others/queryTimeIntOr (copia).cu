...
__global__ void reorderTriple(tripleContainer* src, int maxSize, int mask) {
...
std::vector<mem_t<tripleContainer>*> rdfJoin(tripleContainer* innerTable, int innerSize, tripleContainer* outerTable, int outerSize, JoinMask innerMask[3], JoinMask outerMask[3])
{
	...
	reorderTriple<<<300,1000>>>(outerTable, outerSize, 1);
	cudaDeviceSynchronize();
	gettimeofday(&end, NULL);
	...
	mergesort(outerTable, outerSize , *innerSorter, context);
	
	
	
	//BUG che mi costringe ad invertire inner con outer?
	mem_t<int2> joinResult = inner_join( innerTable, innerSize, outerTable, outerSize,  *innerSorter, context);
