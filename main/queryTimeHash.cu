...
class Query {
	...
	public:
		...
		**/
		...
		void printResults(dense_hash_map<size_t, const char*, std::hash<size_t>, equal_key> &map) {
...
class QueryManager {
	private:
		...
		deviceCircularBuffer devicePointer;
                dense_hash_map<size_t, const char*, std::hash<size_t>, equal_key> map;
	...
	public:
		...
		void start() {
			...
			for (int i =0; i <srcSize; i++) {
				...
				map[currentTriple.subject] = triple[0].c_str();
                                map[currentTriple.predicate] = triple[1].c_str();
                                map[currentTriple.object] = triple[2].c_str();
...
__global__ void unarySelect (circularBuffer<size_t> src, size_t* value, tripleContainer* dest, circularBuffer<tripleContainer> store, int* size) {
		...
		std::vector<int>  arrs) 
{
	...
	for (int i = 0; i < querySize; i++) {
		...
		switch(arrs[i]) {
			...
			case(3): {
				size_t* value1 = &(currentPointer->subject);
				size_t* value2 = &(currentPointer->predicate);
			...
			case(4): {
				size_t* value1 = &(currentPointer->subject);
				size_t* value2 = &(currentPointer->object);
			...
			case(5): {
				size_t* value1 = &(currentPointer->predicate);
				size_t* value2 = &(currentPointer->object);
...
int main(int argc, char** argv) {
		...
		for (int i = 0; i < N_CYCLE; i++) {
			...
			JoinMask outerMask[3];
			outerMask[0] = JoinMask::SBJ;
			outerMask[1] = JoinMask::PRE;
			outerMask[2] = JoinMask::OBJ;
