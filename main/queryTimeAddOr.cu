		...
		std::vector<int>  mask) 
{
	...
	for (int i = 0; i < querySize; i++) {
		...
		//TODO SETTARE DIVISIONE CORRETTA
		int gridSize = 300;
...
std::vector<mem_t<tripleContainer>*> rdfJoin(tripleContainer* innerTable, int innerSize, tripleContainer* outerTable, int outerSize, JoinMask innerMask[3], JoinMask outerMask[3])
{
	...
	TripleComparator* innerSorter = new TripleComparator(innerMask, innerMask);
	TripleComparator* outerSorter = new TripleComparator(outerMask, outerMask);
...
int main(int argc, char** argv) {
		...
		using namespace std;
		struct timeval beginCu, beginEx, end;
		...
		for (int i = 0; i < N_CYCLE; i++) {
			...
			//Create operation object to pass to query manager
