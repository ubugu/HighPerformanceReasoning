...
std::vector<mem_t<tripleContainer<element_t>>*> rdfJoin(tripleContainer<element_t>* innerTable, int innerSize, tripleContainer<element_t>* outerTable, int outerSize, int joinMask[3])
{
	...
	mergesort<launch_params_t<128, 2> >(innerTable, innerSize , *comparator, context);
	mergesort<launch_params_t<128, 2> >(outerTable, outerSize , *comparator, context);
	
	mem_t<int2> joinResult = inner_join<launch_params_t<128, 2> >(innerTable, innerSize, outerTable, outerSize, *comparator, context);	
...
int main(int argc, char** argv) {
		...
		char emptyBuff[MAX_LENGHT] = {0};
                ...
                rdfStoreFile.close();
                float maxEx = 0;
                float minEx = 1000000000;
                float maxCu = 0;
                float minCu = 1000000000;
		...
		cout << "max cuda time " << maxCu << endl;
		cout << "min cuda time " << minCu << endl;
		cout << "max execution time " << maxEx << endl;
		cout << "min execution time " << minEx << endl;
