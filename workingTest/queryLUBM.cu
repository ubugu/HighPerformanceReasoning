...
class CharWrapper
{
	...
	public:
		void setContent(char content[MAX_LENGHT]) {
			std::copy(content, content + MAX_LENGHT, this->content);
		};
			
		__device__ char* getContent() {
			return content;
		};
...
__device__ bool operator < (CharWrapper a, CharWrapper b) 
{	
	return strcasecmp_d(a.getContent(), b.getContent()) < 0;
}            
...
int main(int argc, char** argv) {
		...
		//using tripleElement = CharWrapper;
		...
		char buffer[MAX_LENGHT];
                ...
                for (int i = 0; i < FILE_LENGHT; i++) {
                	...
                	std::vector<string> triple ;
                        ...
                        size = triple[0].size();
                        for (int k = 0; k < MAX_LENGHT + 4; k++) {
                		buffer[k] = ' ';
                	}
                        strncpy(buffer, triple[0].c_str(), size);
                        h_rdfStore[i].subject.setContent(buffer);
                        ...
                        strncpy(buffer, triple[1].c_str(), size);
                        h_rdfStore[i].predicate.setContent(buffer);
                       
                        size = triple[2].size();
                        for (int k = 0; k < MAX_LENGHT + 4; k++) {
                		buffer[k] = ' ';
                	}
                        strncpy(buffer, triple[2].c_str(), size);
                        h_rdfStore[i].object.setContent(buffer);
